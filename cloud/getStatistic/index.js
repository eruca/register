...
exports.main = async(event, context) => {
  ...
  const deadline = `${date.getFullYear()}-${month < 10 ? `0${month}` : month}-${day < 10 ? `0${day}` : day}`;
  ...
  const total = await col.where({ ...yearCond
  }).count();
  const groupTotal = await col.where({ ...yearCond,
    ...groupCond
  }).count();
  const meTotal = await col.where({ ...yearCond,
    ...meCond
  }).count();

  const resultTotal = await col.where({ ...yearCond,
    ...resultCond
  }).count();
  const groupResultTotal = await col.where({ ...yearCond,
    ...groupCond,
    ...resultCond
  }).count();
  const meResultTotal = await col.where({ ...yearCond,
    ...meCond,
    ...resultCond
  }).count();

  const weekTotal = year === '所有' ?
    await col.where({ ...weekCond
    }).count() :
    await col.where({
      enrolltime: _.lt(deadline).and(_.gte(year), _.lt(`${parseInt(year) + 1}`))
    }).count();
  const groupWeekTotal = year === '所有' ?
    await col.where({ ...groupCond,
      ...weekCond
    }).count() :
    await col.where({ ...groupCond,
      enrolltime: _.lt(deadline).and(_.gte(year), _.lt(`${parseInt(year) + 1}`))
    }).count();
  const meWeekTotal = year === '所有' ?
    await col.where({ ...meCond,
      ...weekCond
    }).count() :
    await col.where({ ...meCond,
      enrolltime: _.lt(deadline).and(_.gte(year), _.lt(`${parseInt(year) + 1}`))
